...
$(function(){
	...
	function ReadRoleButton(id){
		g.httpTip.show();
		var url = Base.serverUrl + "uri/queryRoleUriByRoleId";
		var condi = {};
		condi.login_token = g.login_token;
		condi.roleId = id;
		$.ajax({
			...
			success: function(data){
				...
				if(status){
					...
					for(var i= 0 ; i < fieldArr.length ; i++ ){
						html.push('<tr>');
						html.push('<td>' + fieldArr[i].uriColumnValue + '</td>');
						html.push('<td><input type="checkbox" id="' +  fieldArr[i].uriId + '" ' + (fieldArr[i].permission =="1"?"checked":"") + '></td>');
						html.push('</tr>');
					}
					html.push('</table>');
	...
	function saveBtnUp(){
		...
		g.httpTip.show();
		var url = Base.serverUrl + "uri/insertUriRolesByRoleId";
		var condi = {};
		condi.login_token = g.login_token;
		condi.roleId = g.checkedRoleId;
		condi.uriIds = fieldNames;
		$.ajax({
			url:url,data:condi,type:"POST",dataType:"json",context:this,
			success: function(data){
				var status = data.success || false;
