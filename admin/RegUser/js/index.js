...
$(function () {
    ...
    function sendQueryListHttp() {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/customer/findCustomerByQuery";//user/queryCustomerController
        var condi = {};
    ...
    function deleteItem(customerId) {
        if (confirm("你确认删除该用户吗?")) {
            ...
            var url = Base.serverUrl + "pc/customer/deleteCustomerById";//user/deleteCustomerByCustomerIdController
            $.ajax({
                url: url,
                data: condi,
                type: "POST",
    ...
    window.rePass = function(customerId) {
        if (confirm("你确认要重置该用户的密码吗?")) {
            ...
            var url = Base.serverUrl + "pc/customer/resetPassword";//user/updatePasswordDefControlle
            $.ajax({
