...
$(function () {
    ...
    function GetUserInfo() {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/user/findUserById";//user/queryUsersController
        var condi = {};
        condi.login_token = g.login_token;
        condi.userId = g.usersId;
        condi.pageSize = 1;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (status) {
                    var obj = data.obj || [];
                    $("#usersPhone").val(obj.usersPhone);
                    $("#usersName").val(obj.usersName);
                    $("#companyId").val(obj.companyId);
                    FillSubsidiaryId();
                    $("#subsidiaryId").val(obj.subsidiaryId);
                } else {
    ...
    function sendGetCompanyInfoHttp() {
        g.httpTip.show();
        var url = Base.serverUrl + "common/company/findAll";
        var condi = {};
    ...
    function SaveData() {
        ...
        var url = Base.serverUrl + "pc/user/updateUser";//user/updateUsersController
        g.httpTip.show();
        $.ajax({
