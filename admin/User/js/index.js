...
$(function(){
	...
	function sendGetCompanyInfoHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/company/findAll";//order/queryCompanyController
		var condi = {};
		$.ajax({
			url:url,
			data:condi,
	...
	function deleteItem(usersId){
		if(confirm("你确认删除该用户吗?")){
			g.httpTip.show();
			var condi = {};
			condi.userId = usersId;
			condi.login_token = g.login_token;
			var url = Base.serverUrl + "pc/user/deleteUserById";//user/deleteUsersByUsersIdController
			$.ajax({
	...
	window.resetPassword = function(usersId){
		if(confirm("你确认要重置该用户的密码?")){
			...
			condi.userId = usersId;
			condi.login_token = g.login_token;
			var url = Base.serverUrl + "pc/user/resetPassword";//user/resetPassword
			$.ajax({
				url:url,data:condi,type:"POST",dataType:"json",context:this,
				success: function(data){
					//console.log("|",data);
