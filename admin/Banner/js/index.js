...
$(function(){
	...
	function sendGetNavigationKeyHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/dic/findByParents";//bannerImage/getNavigationKey
		var condi = {};
		condi.parents = "1032";
		$.ajax({
			url:url,
	...
	function sendQueryListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/bannerImage/findByQuery";//bannerImage/queryBannerImageController
		var condi = {};
	...
	function changeBannerUsedFlag(status,bmId){
		...
		if(confirm(msg)){
			...
			if(status == 1){
				var url = Base.serverUrl + "common/bannerImage/openBannerImage";//bannerImage/usedBannerImage
			}
			else{
				var url = Base.serverUrl + "common/bannerImage/stopBannerImage";//bannerImage/stopBannerImage
			}
			$.ajax({
				url:url,
				data:condi,
	...
	function deleteItem(bmId){
		if(confirm("你确认删除该轮播图吗?")){
			...
			var url = Base.serverUrl + "common/bannerImage/deleteBannerImage";//bannerImage/deleteBannerImage
			$.ajax({
				url:url,
				data:condi,
				type:"POST",
