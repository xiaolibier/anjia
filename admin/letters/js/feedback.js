...
$(function(){
	...
	function sendQueryListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "pc/communication/findFeedbackByQuery";
		var condi = {};
	...
	function changeListHtml(data){
		...
		for(var i = 0,len = obj.length; i < len; i++){
			...
			html.push('<td>' + (d.customerPhone || "") + '</td>');
			html.push('<td>' + (d.direct == '0'? '出' : '进' || "") + '</td>');
			if(dr == '0'){
			html.push('<td><a href="javascript:changeBannerUsedFlag(\'' + (d.id || "") + '\');">撤回</a></td>');
			}else if(dr == '1'){
				html.push('<td><a href="javascript:newItem('+(d.customerPhone || "")+');">回复</a></td>');
	...
	function newItem(customerPhone){
		location.href = "answer.html?customerPhone="+customerPhone;
	}
