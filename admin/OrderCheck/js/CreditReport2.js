...
$(function(){
	...
	function sendQueryRiskOrderListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "pc/zccredit/findByOrderId";//credit/getCreditLoanInfoByOrderId
		var condi = {};
	...
	function changeOrderListHtml(data){
		...
		html.push('<h4 style="text-align: center">查询结果[' + data.obj.loanRecords.length + ']条数据</h4><br><table class="table table-bordered table-hover definewidth m10" >');
		html.push('<tr>');
		html.push('<th>90天以上逾期次数(次)</th>');
		html.push('<th>180天以上逾期次数(次)</th>');
		html.push('<th>借款逾期次数(次)</th>');
		html.push('</tr>');
		html.push('<tr>');
		html.push('<td>' + (s.firstOverdueTimes || 0) + '</td>');
		html.push('<td>' + (s.secondOverdueTimes || 0) + '</td>');
		html.push('<td>' + (s.overdueTimes || 0) + '</td>');
		html.push('</tr>');
		html.push('</table>');
		
		html.push('<table class="table table-bordered table-hover definewidth m10" ><thead>');
		html.push('<tr>');
		html.push('<th>借款时间</th>');
		html.push('<th>借款金额</th>');
		html.push('<th>审批结果</th>');
		html.push('<th>审批结果编码</th>');
		html.push('<th>目前状态</th>');
		html.push('<th>目前状态编码</th>');
		html.push('</tr>');
		var obj = data.obj.loanRecords || [];
		for(var i = 0,len = obj.length; i < len; i++){
			var d = obj[i];
			html.push('<tr>');
			html.push('<td>' + (d.loanTime || "") + '</td>');
			html.push('<td>' + (d.amount || "") + '</td>');
			html.push('<td>' + (d.approveStatus || "") + '</td>');
			html.push('<td>' + (d.approveStatusCode || "") + '</td>');
			html.push('<td>' + (d.currentStatus || "") + '</td>');
			html.push('<td>' + (d.currentStatusCode || "") + '</td>');
