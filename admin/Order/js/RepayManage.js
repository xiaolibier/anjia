...
$(function () {
    ...
    function sendQueryOrderListHttp() {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/findRepaymentOrder";//order/queryRepaymentOrdersController
        var condi = {};
    ...
    function changeOrderListHtml(data) {
        ...
        for (var i = 0, len = obj.length; i < len; i++) {
            ...
            if (d.status == "100507") {
                ...
                aStr = '<a href="javascript:void(0)" onclick="YiCiHuanQing(' + d.totalResidueBalance + ',\'' + d.orderId + '\')">一次还清</a>';
			...
			html.push('<td>' + (d.realPoundage || 0) + '</td>');
    ...
    function CreateSubHtml(paramObj, TdObj, OrderStatus) {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/findRepaymentRecordByOrderId";//order/queryRepaymentOrderDetail
        $.ajax({
    ...
    window.SaveDhk = function () {
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/helpPayOverdue";//order/helpPayOverdueFenqi
        var condi = {};
        condi.login_token = g.login_token;
    ...
    window.SaveYchq = function () {
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/helpPayFinish";//order/updateRepaymentRecordsByOncepaidFinish
        var condi = {};
        condi.login_token = g.login_token;
        condi.orderId = OrderId;
        condi.realRepaymentTime = realRepaymentTime.val();
    ...
    window.ZhongZhi = function () {
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/finishContract";//order/finishContractController
        var condi = {};
        condi.login_token = g.login_token;
        condi.orderId = OrderId;
        condi.finishContractReason = Reason.val();
        $.ajax({
            url: url, data: condi, type: "POST", dataType: "json", context: this,
            success: function (data) {
    ...
    window.SafeFqlb_Dhk = function () {
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/helpPayFenqi";//order/helpPayFenqi
        var condi = {};
        condi.login_token = g.login_token;
    ...
    window.SMSTips = function (OrderId, repaymentRecordId) {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/message/sendRepaymentMessage";//order/sendRepaymentMessage
        var condi = {};
        condi.login_token = g.login_token;
        condi.orderId = OrderId;
        condi.repaymentRecordId = repaymentRecordId;
    ...
    window.SaveHuoMian = function () {
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/exemptOrder";//order/exemptOrder
        var condi = {};
        condi.login_token = g.login_token;
        condi.orderId = OrderId;
    ...
    window.OutXls = function () {
        var ParamObj = {};
        ParamObj.login_token = g.login_token;
        Hmgx.serializeDownload(Base.serverUrl + "pc/order/findRepaymentOrderExport", "CX", ParamObj);
    }
});
