...
$(function(){
	...
	function sendGetRepayOrderInfoListHttp(orderId){
		...
		g.httpTip.show();
		var url = Base.serverUrl + "pc/order/findLoanRecordByOrderId";//order/queryLoanRecordByQuery
		$.ajax({
			url:url,
			data:condi,
			type:"POST",
	...
	function loanByLoanRecord(loanRecordId,loanMaxMoney, loanResidueMoney,loanTimes){
		...
		g.httpTip.show();
		var url = Base.serverUrl + "pc/order/merchantCollectFund";//order/loanByLoanRecord
		$.ajax({
			url:url,
			data:condi,
			type:"POST",
