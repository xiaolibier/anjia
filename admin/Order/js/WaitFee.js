...
$(function () {
    ...
    function sendQueryOrderListHttp() {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/findPoundagePayOrder";//order/queryOrdersController
        var condi = {};
    ...
    function changeOrderListHtml(data) {
        ...
        for (var i = 0, len = obj.length; i < len; i++) {
            ...
            html.push('<td>' + d.realPoundage + '</td>');
            ...
            /* if (status == "100505") { */
                ...
                var buttonStr = '<a class="" href="javascript:ShowWin(\'' + d.orderId +  '\',\'' + d.customerId + '\',\'' + d.poundage + '\')">代缴费</a>';
    ...
    window.DaiJiaoFei = function(id){
        ...
        if(confirm("你确认要执行代交操作吗?")) {
            ...
            condi.login_token = g.login_token;
            var url = Base.serverUrl + "pc/order/helpPayPoundage";//order/helpPayPoundage
            $.ajax({
                url: url, data: condi, type: "POST", dataType: "json", context: this,
                success: function (data) {
                    var status = data.success || false;
