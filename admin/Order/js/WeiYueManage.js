...
$(function () {
    ...
    function sendQueryOrderListHttp() {
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/findBreakContractOrder";//order/getBreakContractOrdersController
        var condi = {};
    ...
    function changeOrderListHtml(data) {
        ...
        for (var i = 0, len = obj.length; i < len; i++) {
            ...
            html.push('<td>' +  (d.residuePrincipal || 0 ) + '元</td>');
            ...
            html.push('<td>' + (d.overdueDay ||"" )+ '</td>');
            html.push('<td>' + (d.overdueFee ||"" )+ '</td>');
            html.push('<td>' + (d.residueBalance ||"" )+ '</td>');
            ...
            if(d.status == "100511"){
                html.push('<td width="300"><a href="javascript:Hmgx.openWin(\'ViewOrderDetail.html?orderid=' + d.orderId + '\')">查看</a>&nbsp;&nbsp;<a href="javascript:SMSTips('+ d.orderId +')">提醒</a>&nbsp;&nbsp;<a href="javascript:popFqlb_dhkWin(' + d.residueBalance + ',' + d.orderId +')">代还款</a></td>');
            }else{
    ...
    window.SMSTips=function(OrderId){
        g.httpTip.show();
        var url = Base.serverUrl + "pc/message/sendBreakContractMessage";//order/sendBreakContractRepaymentMessage
        var condi = {};
        condi.login_token = g.login_token;
    ...
    window.SafeFqlb_Dhk = function(){
        ...
        g.httpTip.show();
        var url = Base.serverUrl + "pc/order/helpPayBreakContract";//order/helpPayBreakContractOrder
        var condi = {};
        condi.login_token = g.login_token;
        condi.orderId = OrderId;
    ...
    window.OutXls = function(){
        ...
        Hmgx.serializeDownload(Base.serverUrl  + "pc/order/findBreakContractOrderExport","CX",ParamObj);
    }
});