...
$(function(){
	...
	function ReadRoleFild(id){
		g.httpTip.show();
		var url = Base.serverUrl + "common/auth/findAllRoleAuthorityByRoleId";//orderRole/queryOrderRolesByRoleId
		var condi = {};
		condi.login_token = g.login_token;
		condi.roleId = id;
		condi.authoritySystem= 102301;
		condi.authorityType= 102202;
		$.ajax({
			url:url,data:condi,type:"POST",dataType:"json",context:this,
			success: function(data){
				var status = data.success || false;
	...
	function saveBtnUp(){
		...
		g.httpTip.show();
		var url = Base.serverUrl + "common/auth/insertRoleAuthority";//orderRole/insertOrderRolesByRoleId
		var condi = {};
		condi.login_token = g.login_token;
