...
$(function(){
	...
	$("#bindbtn").bind("click",bindRoleBtnUp);

	function addNewPage(){
		location.href = "add.html";
	}

	function bindRoleBtnUp(){
		var ckrole = $("input[name='role']:checked");
		var roleId = $("#rolelist").val() || "";
		if(ckrole.length > 0 && roleId !== ""){
			var userId = ckrole[0].id;
			var condi = {};
			condi.login_token = g.login_token;
			condi.roleId = roleId;
			condi.userId = userId;
			sendBindUserRoleHttp(condi);
		}
		else{
			Utils.alert("请选择用户");
		}
	}
	function sendBindUserRoleHttp(condi){
		g.httpTip.show();
		var url = Base.serverUrl + "pc/user/insertUserRole";//userRole/insertUserRole
		$.ajax({
			url:url,
			data:condi,
			type:"POST",
			...
			success: function(data){
				console.log("sendBindUserRoleHttp",data);
	...
	function sendGetCompanyInfoHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/company/findCompanys";//order/queryCompanyController
		var condi = {};
		$.ajax({
			url:url,
			data:condi,
	...
	function sendGetRoleListHttp(){
		...
		$.ajax({
			...
			context:this,
			success: function(data){
				console.log("sendGetRoleListHttp",data);
				var status = data.success || false;
				if(status){
					var obj = data.obj || [];
					changeRoleSelectHtml(obj);
	...
	function changeRoleSelectHtml(obj){
		var data = obj || {};
		var option = [];
		for(var i = 0,len = data.length; i < len; i++){
			var d = data[i];
		...
		$("#rolelist").html(option.join(''));
	}

	function sendQueryListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "pc/user/findUserByQuery";//user/queryUsersController
		var condi = {};
		condi.login_token = g.login_token;
	...
	function changeListHtml(data){
		...
		html.push('<tr>');
		html.push('<th>选择账户</th>');
		html.push('<th>账户角色</th>');
		...
		for(var i = 0,len = obj.length; i < len; i++){
			...
			if(deleted === 1){
				...
				html.push('<td><input id="'+ usersId + '" type="radio" name="role" /></td>');
			}
