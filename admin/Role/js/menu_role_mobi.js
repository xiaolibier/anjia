...
$(function(){
	...
	function sendQueryRoleListHttp(){
		...
		$.ajax({
			...
			success: function(data){
				//console.log("sendQueryRoleListHttp",data);
	...
	function roleClick(evt){
		...
		sendQueryTreeByRoleIdHttp(id);
	}

	function sendQueryTreeByRoleIdHttp(id){
		g.httpTip.show();
		var url = Base.serverUrl + "common/auth/findAuthorityByRoleId";//authority/phone/queryAdminMenuByRoleId
		var condi = {};
		condi.login_token = g.login_token;
		condi.roleId = id;
		...
		$.ajax({
			...
			context:this,
			success: function(data){
				//console.log("sendQueryTreeByRoleIdHttp",data);
				var status = data.success || false;
				if(status){
					var _data = format.init(g.alldata,data);
	...
	function saveBtnUp(){
		sendInsertTreeByRoleIdHttp();
	}

	function sendInsertTreeByRoleIdHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/auth/insertRoleAuthority";//roleAuthority/phone/insertAdminMenuRoleAuthority
		var condi = {};
		condi.login_token = g.login_token;
		condi.roleId = g.checkedRoleId;
		condi.authorityIds = g.checkMenus;
		condi.authoritySystem= 102302;
		condi.authorityType= 102201;
		$.ajax({
			url:url,
			...
			context:this,
			success: function(data){
				//console.log("sendInsertTreeByRoleIdHttp",data);
				var status = data.success || false;
				if(status){
					Utils.alert("保存成功");
				}
	...
	**/
	var format = {
		format_children:function(__data,_selected_data){
			var arr = new Array();
			for (var i = 0; i<__data.length ; i++) {
				...
				obj["children"]=this.format_children(__data[i].childrenAuthority,_selected_data);
				obj["open"]=true;
				for (var j = 0; j < _selected_data.length; j++) {
					if (obj["authorityId"]==_selected_data[j]["authorityId"]) {
						obj["checked"] =true;
						break;
					};
				};
		...
		init:function(data,selected_data){
			if (data.success) {
				...
				_data[0].children = this.format_children(data.obj.childrenAuthority,_selected_data);
				return _data;
			}else{
				alert("菜单数据错误!");
				return null;
	...
	function sendQueryListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "common/auth/findMenuTree";//authority/phone/queryAdminMenuTree
		var condi = {};
		condi.login_token = g.login_token;
		condi.authoritySystem= 102302;
		condi.authorityType= 102201;
