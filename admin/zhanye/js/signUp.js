...
$(function(){
	...
	function sendQueryOrderListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "pc/designer/findByQuery";
		var condi = {};
		condi.login_token = g.login_token;
		condi.currentPageNum = g.currentPage;
		condi.name = $("#applicantName").val() || "";
		condi.phone = $("#applicantPhone").val() || "";
	...
	function changeOrderListHtml(data){
		...
		for(var i = 0,len = obj.length; i < len; i++){
			...
			html.push('<td>' + (d.phone || "") + '</td>');
			html.push('<td>' + (d.code || "") + '</td>');
			html.push('<td>' + (d.statusDesc || "") + '</td>');
			html.push('<td><a href="javascript:void(0)" onclick="confir(\'' + id +'\')">审批</a></td>');
			html.push('</tr>');
		}
		html.push('</table>');
	...
	window.confir = function(id){
		if(!confirm("是否通过审核？")){return;}
        if(id==""){
            alert("非法请检查！");
            return false;
        }
        var url = Base.serverUrl + "pc/designer/passDesigner";
        var condi = {};
        condi.login_token = g.login_token;
        condi.id = id;
        $.ajax({
