...
$(function () {
    ...
    function ReadOrderInfo() {
        ...
        $.ajax({//处理字段、图片、并存放全局变量
            ...
            url: Base.serverUrl + "md/order/findAuthOrderByOrderId",
            ...
            success: function (data) {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    if ($("#" + column).length != 1) {
                        ...
                        $("#" + column).val(columnValue); //赋值
                        $("#" + column).parents("tr").addClass("show");//显示
                        if(column == "poundageRepaymentType"){
                            changePoundageRepaymentType( columnValue );//改变 服务费支付方式 显示的 TR
                        }
                    }
                }
                //图片
    ...
    function ShowPic(picData) {
        for (var i = 0; i < picData.length; i++) {
            ...
            if (ImgType == 100713) {//资源文件
                ...
                divHtml += "<a href='" + ImgUrl + "' target='_blank' title='点击下载此资源文件'><img src='../res/img/Save.png' style='margin: 5px'></a>";
                divHtml += "<img src='../res/img/del.png' width='20' height='20' onclick='DelPic(" + orderMaterialId + ")' style='margin: 5px; vertical-align: bottom;cursor: pointer '>";
                divHtml += "</div>";
                $("#Img" + ImgType).append(divHtml);
                $("#Img" + ImgType).parents("tr").addClass("show");//显示
            } else {
