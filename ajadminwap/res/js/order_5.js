...
$(function(){
	...
	function sendQueryOrderListHttp(sys){
		g.httpTip.show();
		var url = Base.serverUrl + "md/order/findLoanRecordByQuery";
		var condi = {};
		...
		if(sys){
			...
			condi.realLoanTime = $("#realLoanTime").val() || "";
		}
		$.ajax({
			url:url,
			data:condi,
	...
	function changeOrderListHtml(data){
		...
		html.push('<td>支付金额</td>');
		html.push('<td>支付时间</td>');
		html.push('</tr>');
		var obj = data.list || [];
