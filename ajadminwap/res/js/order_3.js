...
$(function(){
	...
	function sendQueryOrderListHttp(sys){
		g.httpTip.show();
		var url = Base.serverUrl + "md/order/findFinanceApprovingOrder";
		var condi = {};
	...
	function changeOrderListHtml(data){
		...
		for(var i = 0,len = obj.length; i < len; i++){
			...
			html.push('<td>' + confirmTime + '</td>');
			html.push('<td><a href="javascript:lending (\''+orderId+'\')" class="common_a_btn">放款</a></td>');
			html.push('</tr>');
		}
		html.push('</table>');
	...
	/* 放款 */
	function lending(orderId){
		var orderId = orderId || "";
		var url = Base.serverUrl + "md/order/financeLoan";
		var condi = {};
		condi.login_token = g.login_token;
		condi.orderId = orderId;
		_alert('确认要放款吗？',function(){
			$.ajax({
				url:url,
				data:condi,
				type:"POST",
				...
				success: function(data){
					var status = data.success || false;
					if(status){
						c_alert('放款成功！',function(){location.reload();},function(){});
					}
					else{
