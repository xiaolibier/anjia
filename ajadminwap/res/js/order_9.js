...
$(function(){
	...
	function sendQueryOrderListHttp(sys){
		g.httpTip.show();
		var url = Base.serverUrl + "md/order/findWaitForConfirmOrder";
		var condi = {};
	...
	function changeOrderListHtml(data){
		...
		html.push('<td>审批金额</td>');
		html.push('<td>审批期数</td>');
		html.push('<td>服务费支付方式</td>');
		html.push('<td>操作</td>');
		html.push('</tr>');
		var obj = data.list || [];
		for(var i = 0,len = obj.length; i < len; i++){
			...
			html.push('<td>' + packageMoney + '</td>');
			html.push('<td>' + fenQiTimes + '</td>');
			html.push('<td>' + poundageRepaymentTypeDesc + '</td>');
			html.push('<td><a href="javascript:accept(\''+orderId+'\')" class="common_a_btn">接受</a> <a href="javascript:cancel(\''+orderId+'\')" class="common_a_btn">取消</a></td>');
			html.push('</tr>');
		}
		html.push('</table>');
		html.push('</div>');
