...
$(function(){
	...
	function sendQueryOrderListHttp(){
		g.httpTip.show();
		var url = Base.serverUrl + "md/order/firstApproveOrderReport";
		var condi = {};
	...
	function changeOrderListHtml(data){
		...
		html.push('<div><span class="text">初审报告</span><textarea id="firstApproveReport" class="common_area">'+firstApproveReport+'</textarea></div>');
		html.push('<div><span class="text">初审意见</span><textarea id="firstApproveRemarks" class="common_area">'+firstApproveRemarks+'</textarea></div>');
		html.push('<div class="rca_textarea_1" style="display:none;"><span class="text">复审报告</span><textarea id="fk2_approve_report" class="common_area"></textarea></div>');
	...
	window.first_approveOrder = function(approveResult){
		var url = Base.serverUrl + "md/order/firstApproveOrder";
		var condi = {};
		condi.packageMoney = $("#packageMoney").val() || 0;
		condi.fenQiTimes = $("#fenQiTimes").val() || "";
		condi.approveReport = $("#firstApproveReport").val() || "";
		condi.approveRemarks = $("#firstApproveRemarks").val() || "";
		condi.orderId = g.orderId;
		condi.login_token = g.login_token;
