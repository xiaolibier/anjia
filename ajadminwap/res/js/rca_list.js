...
$(function(){
	...
	function sendQueryOrderListHttp(sys){
		g.httpTip.show();
		var url = Base.serverUrl + "md/order/findRiskManagementOrder";
		var condi = {};
	...
	function changeOrderListHtml(data){
		...
		for(var i = 0,len = obj.length; i < len; i++){
			...
			html.push('<td>' + (d.lastFkApproveType || "") + " " + getCheckResult(d.lastFkApproveResult) +  '</td>');
			html.push('<td>' + applicantName + '</td>');
			html.push('<td><a href="javascript:changeOrder(\''+orderId+'\')" class="common_a_btn">修改信息</a> <a href="javascript:checkOrder(\''+orderId+'\')" class="common_a_btn">审核意见</a></td>');
			html.push('</tr>');
		}
		html.push('</table>');
	...
	window.getCheckResult = function(CheckCode){
		switch (CheckCode){
			case "0":
				return "通过";
			case "1":
	...
	//修改信息
	window.changeOrder = function(orderId){
		var orderId = orderId || "";
		var money = money || 0;
		location.href="../html/ModifyOrder_list.html?orderId="+orderId;
	}
	//审核意见
	window.checkOrder = function(orderId){
		var orderId = orderId || "";
		var money = money || 0;
		location.href="../html/rca_audit_list.html?orderId="+orderId;
	}
